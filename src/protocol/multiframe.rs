...
mod tests {
    ...
    fn mutltiframe_allows_interspersed_frames() {
        ...
        assert_eq!(format!("{:?}", actions), "[BeginMultiFrameMessage { header: [Request c:0 id:0], payload: 1361 bytes }, SendSingleFrame { header: [err:Other c:1 id:42188], payload: 10 bytes }]");
    ...
    fn mutltiframe_does_not_allow_multiple_multiframe_transfers() {
        ...
        // Failed sequence was generated by a proptest, check that it matches.
        assert_eq!(format!("{:?}", actions), "[BeginMultiFrameMessage { header: [Request c:0 id:0], payload: 12 bytes }, SendConflictingMultiFrameMessage { channel: ChannelId(0), id: Id(1), payload: 106 bytes }]");
